...
def _ensure_dirs():
    for d in [DIR_OVERVIEW, DIR_DAILY, DIR_WEEKLY, DIR_DISTRICTS,
              DIR_HYPOTHESES, DIR_WATCHLIST, DIR_BIZ_CIRCLE]:
...
def append_district_observation(region: str, observation: str):
    """向区域档案正文追加一条按日期标记的观察。"""
    profile = read_district_profile(region)
    body = profile["body"]
...
def append_biz_circle_observation(biz_circle: str, observation: str):
    """向板块档案正文追加一条按日期标记的观察。"""
    _ensure_dirs()
...
def write_buying_assessment(assessment: dict):
    """覆写购买力评估文件。"""
    _ensure_dirs()
    path = REPORTS_DIR / DIR_OVERVIEW / "购买力评估.md"
    today = date.today().isoformat()
